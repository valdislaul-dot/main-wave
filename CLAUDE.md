...
### 买入开关（2026-08-20新增，3年数据驱动分档）
- **弱市**(涨停<60或最高≤2板): 空仓; 升温日(涨停数较前日回升)例外1/3仓
- **正常**(60-109): 半仓 | **强势**(≥110): 全仓
- 数据: 涨停数10只分档扫描3年 — 期望转正线~70只, 稳定为正110+;
  <60档期望-0.2%~-2.0% | 60-109档-0.2%~+0.7% | 110+档+1.9%~+3.0%
- 竞价二次确认: 池均gap≤-0.5% → 环境降一档 (3年724日校准, -2%阈值从未触发已弃)
- 最高≤2板条件: 08-24二维扫描验证冗余(60+涨停时几乎必>2板), 防御性保留
...
**目标**: 跌停风险纳入V3评分，作为第10个因子（扣分项）
**数据基础**: 3年全路径积分模型（scripts/dt_risk_integrated.py）— T日状态→P(触跌停)/风险期望
**初步方案**（扣分档待解冻后回测调参）:
- 查表: 按T日板数+量比+封板状态 → 触跌停概率
  - <5%: 0分 | 5-10%: -3 | 10-20%: -6 | 20-30%: -9 | ≥30%: -12
- 实施步骤: ① 推荐回看N≥50确认现有因子地基 ② 加分档回测(在回看历史数据上验证扣分是否提升排序质量) ③ 配置进scoring_config.json
