...
**What goes wrong:** task boots a nonexistent `C:\Users\Davis\Desktop\项目\gogo\run_api.bat` → LastTaskResult failure at every reboot; service never comes up; success criterion 2 fails silently.
...
- Hardcoded `BASE=C:\Users\Davis\Desktop\项目\gogo` in `auto_start.bat`/`install_scheduled_task.ps1`: stale since the repo lives at `Desktop\gogo`; never replicate (D-09).
