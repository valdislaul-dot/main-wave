...
for name,code in sm.items():
    ...
    for i,k in enumerate(klines):
        ...
        e={'open':o,'close':c,'high':h,'low':l,'is_limit_up':False,'prev_close':pc,'gap_open_pct':0,'lu_price':0}
        if pc and pc>0:
            e['is_limit_up']=lu(c,pc,lpt); e['gap_open_pct']=(o-pc)/pc*100
            e['lu_price']=round(pc*(1+lpt),2)
        pdb[name][dt]=e; pc=c
tl=[]
...
out_path = r'C:\Users\Davis\Desktop\项目\gogo\logs\executable_sell.txt'
with open(out_path, 'w', encoding='utf-8') as f:
    f.write("可执行卖出: 涨停日挂涨停价卖 / 不涨停日开盘卖\n")
    f.write("="*65+"\n\n")
    f.write(f"{'仓位':<8} {'阈值':<6} {'最终资产':>12} {'收益率':>10} {'距1022k':>10} {'胜率':>6}\n")
    f.write("-"*60+"\n")

    for pos_pct in [0.55, 1.0]:
        for threshold in [6, 5, 4, 3]:
            ...
            for didx,date in enumerate(ad):
                ...
                if rec!='休息' and len(pos_list)==0:
                    ...
                    if bn in pdb and date in pdb[bn]:
                        bp_=pdb[bn][date]['open']; deploy=cash*pos_pct
            ...
            mark=' <-- CLOSE' if abs(diff)<80000 else ''
            f.write(f"{pos_pct*100:>4.0f}%  {threshold}%    {final:>12,.0f} {ret:>+9.1f}% {diff:>+10,.0f} {wr:>5.0f}%{mark}\n")

    f.write("\n结论: 涨停日卖涨停价+不涨停日卖开盘, 完全可执行\n")
