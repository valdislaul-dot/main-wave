"""
阈值敏感性分析：开盘涨幅阈值对模型收益的影响
"""
import json, os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), 'daily'))
...
import openpyxl
wb = openpyxl.load_workbook(os.path.join(BASE, 'data', '副本主升浪.xlsx'))
ws = wb['Sheet1']
records = []
...
def lu(c, pc, lpct):
    ...
    return c >= round(pc * (1 + lpct), 2) - 0.005
...
for name, code in sm.items():
    ...
    for i, k in enumerate(klines):
        ...
        e = {'open': o, 'close': c, 'high': h, 'low': l, 'volume': v,
...
out_path = os.path.join(BASE, 'logs', 'threshold_analysis.md')
with open(out_path, 'w', encoding='utf-8') as f:
    f.write("# 开盘涨幅阈值敏感性分析\n\n")
    f.write("## 方法\n\n")
    f.write("固定参数: 55%仓位, 模型选股(评分≥30), HIGH价卖出, 低开阈值0%\n")
    f.write("变量: 开盘涨幅阈值 (3%~10%)\n")
    f.write("样本: 交易员77笔交易数据, 2026-03-03 至 2026-07-24\n\n")
    f.write("## 结果\n\n")
    f.write("| 阈值 | 最终资产 | 收益率 | 交易笔数 | 胜率 | 最大回撤 | 夏普近似 |\n")
    f.write("|------|----------|--------|----------|------|----------|----------|\n")

    all_results = []
    for strength in [10, 9.5, 9, 8.5, 8, 7.5, 7, 6.5, 6, 5.5, 5, 4.5, 4, 3.5, 3]:
        ...
        # Simple Sharpe: avg daily return / std, annualized
        if len(tlist) >= 5:
            avg_pnl = sum(t['pnl'] for t in tlist) / len(tlist)
            std_pnl = (sum((t['pnl'] - avg_pnl) ** 2 for t in tlist) / len(tlist)) ** 0.5
            sharpe = (avg_pnl / std_pnl * (len(tlist) ** 0.5)) if std_pnl > 0 else 0
        else:
            sharpe = 0

        all_results.append({
            'th': strength, 'final': final, 'ret': ret, 'nt': len(tlist),
            'wr': wr, 'dd': max_dd, 'sharpe': sharpe
        })

        marker = ' **← 最优**' if abs(final - 1000000) < 30000 else ''
        f.write(f"| {strength}% | {final:>10,.0f} | {ret:>+6.1f}% | {len(tlist)} | {wr:.0f}% | {max_dd:.1f}% | {sharpe:.2f} |{marker}\n")

    # Recommendation
    best = min(all_results, key=lambda x: abs(x['final'] - 1000000))

    f.write("\n## 建议\n\n")
    f.write(f"**回测最优阈值: {best['th']}%** (最接近100万目标)\n\n")
    f.write("### 阈值选择指南\n\n")
    f.write("| 市场环境 | 推荐阈值 | 逻辑 |\n")
    f.write("|----------|----------|------|\n")
    f.write("| 强势市场(连板率高) | 7-8% | 更耐心, 让利润跑 |\n")
    f.write("| 正常市场 | 5-6% | 平衡持有与止损 |\n")
    f.write("| 弱势市场(断板率高) | 3-4% | 更快离场, 保本金 |\n\n")
    f.write("### 阈值越低 → 卖得越快 → 交易越频繁 → 胜率越低 → 但单笔亏损越小\n")
    f.write("### 阈值越高 → 卖得越慢 → 持有越久 → 可能捕获大波段 → 也可能被套\n")
    f.write("\n*分析基于2026-03至2026-07回测数据，实盘需根据行情调整*\n")
