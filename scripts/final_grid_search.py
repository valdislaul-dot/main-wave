...
tl=[]
for s,st in records:
    d=ed(s); tl.append((d.strftime('%Y-%m-%d'),st))
...
with open(out_path,'w',encoding='utf-8') as f:
    ...
    # Best calibration approach: parameterize the hybrid model with precise thresholds
    # Search: hard_stop, trailing_stop, gap_exit, stale_days
    results=[]

    for hard_stop in [-5,-7,-8,-10,-12]:
        for trail_stop in [-3,-5,-8,-10]:
            for gap_exit in [0,-1,-2,-3]:
                for stale_days in [1,2,3,5]:
                    for sp_fn,sp_lbl in [(sp_v,'VWAP'),(sp_c,'CLOSE')]:
                        ...
                        for didx,date in enumerate(ad):
                            ...
                            for pos in pos_list[:]:
                                ...
                                if ip and ip['is_limit_up']:
                                    pos['dsl']=0
                                    pos['pkc']=max(pos.get('pkc',0),it['close'])
                                    continue

                                pos['dsl']=pos.get('dsl',0)+1
                                pos['pkc']=max(pos.get('pkc',it['close']),it['close'])

                                should_sell=False; sp=None

                                # Hard stop
                                loss=(it['open']-bp_)/bp_*100
                                if loss<=hard_stop:
                                    should_sell=True; sp=it['open']

                                # Trailing stop
                                if not should_sell and pos.get('pkc',0)>0:
                                    dd_=(it['close']-pos['pkc'])/pos['pkc']*100
                                    if dd_<=trail_stop:
                                        should_sell=True; sp=sp_fn(date,n)

                                # Gap exit
                                if not should_sell and ip:
                                    pcv=ip['close']
                                    if pcv>0:
                                        gap=(it['open']-pcv)/pcv*100
                                        if gap<gap_exit and pos['dsl']>=1:
                                            should_sell=True; sp=sp_fn(date,n)

                                # Stale exit
                                if not should_sell and pos['dsl']>=stale_days:
                                    should_sell=True; sp=sp_fn(date,n)
                        ...
                        final=cash+pv
                        ret=(final-INIT)/INIT*100; diff=final-TARGET
                        ntr=len(tlist)
                        wr_=sum(1 for t in tlist if t['pnl']>0)/max(ntr,1)*100
                        results.append({
                            'hs':hard_stop,'ts':trail_stop,'ge':gap_exit,
                            'sd':stale_days,'sp':sp_lbl,'final':final,'diff':diff,
                            'ntr':ntr,'wr':wr_,'ret':ret
                        })

    results.sort(key=lambda x: abs(x['diff']))

    f.write(f"{'硬止损':>6} {'移动止盈':>8} {'低开退出':>8} {'不板天数':>8} {'卖出价':<8} {'最终资产':>12} {'距目标':>10} {'交易':>5} {'胜率':>6}\n")
    f.write("-"*85+"\n")
    for r in results[:30]:
        f.write(f"{r['hs']:>+5}% {r['ts']:>+7}% {r['ge']:>+7}% {r['sd']:>8} {r['sp']:<8} {r['final']:>12,.0f} {r['diff']:>+10,.0f} {r['ntr']:>5} {r['wr']:>5.0f}%\n")

    # Best match
    best=results[0]
    f.write(f"\n\n最佳匹配:\n")
    f.write(f"  硬止损={best['hs']}% | 移动止盈={best['ts']}% | 低开退出={best['ge']}% | 不板天数={best['sd']} | 卖出价={best['sp']}\n")
    f.write(f"  最终={best['final']:,.0f} | 距目标={best['diff']:+,.0f} ({best['diff']/TARGET*100:+.1f}%) | 交易={best['ntr']}笔 | 胜率={best['wr']:.0f}%\n")

    # Apply best rules to MODEL picks
    f.write(f"\n\n用最佳规则 + 模型选股:\n")
    # ... simplified re-run
    # Score function (abbreviated)
    ...
    # Re-run with best params + model picks
    sp_fn_best=sp_v if best['sp']=='VWAP' else sp_c
    pos_list=[]; cash=INIT; tlist=[]
    for didx,date in enumerate(ad):
        rec=dict(tl).get(date,'休息')
        pd_=ad[didx-1] if didx>0 else None
        ...
        for pos in pos_list[:]:
            ...
            it=pdb[n][date]; ip=pdb[n].get(pd_) if pd_ else None
            if ip and ip['is_limit_up']: pos['dsl']=0; pos['pkc']=max(pos.get('pkc',0),it['close']); continue
            pos['dsl']=pos.get('dsl',0)+1
            pos['pkc']=max(pos.get('pkc',it['close']),it['close'])
            should_sell=False; sp=None
            loss=(it['open']-bp_)/bp_*100
            if loss<=best['hs']: should_sell=True; sp=it['open']
            if not should_sell and pos.get('pkc',0)>0:
                dd_=(it['close']-pos['pkc'])/pos['pkc']*100
                if dd_<=best['ts']: should_sell=True; sp=sp_fn_best(date,n)
            if not should_sell and ip:
                pcv=ip['close']
                if pcv>0:
                    gap=(it['open']-pcv)/pcv*100
                    if gap<best['ge'] and pos['dsl']>=1: should_sell=True; sp=sp_fn_best(date,n)
            if not should_sell and pos['dsl']>=best['sd']: should_sell=True; sp=sp_fn_best(date,n)
            if should_sell and sp and sp>0:
                tlist.append({'pnl':(sp-bp_)/bp_*100})
    ...
    f.write(f"  最终={mfinal:,.0f} | 收益={mret:+.1f}% | 交易={len(tlist)}笔 | 胜率={mwr:.0f}%\n")
    ...
    f.write("最终总结\n")
    f.write(f"{'='*80}\n\n")
    f.write(f"交易员卖出规则推断 (使用交易员自己的选股达到最佳拟合):\n")
    f.write(f"  1. 连板持有: 前日涨停 → 坚决不卖\n")
    f.write(f"  2. 硬止损: 买入后跌幅超过{abs(best['hs'])}% → 开盘价卖出\n")
    f.write(f"  3. 移动止盈: 从最高收盘价回撤超过{abs(best['ts'])}% → {best['sp']}价卖出\n")
    f.write(f"  4. 低开退出: 前日不涨停+今日低开超过{abs(best['ge'])}% → {best['sp']}价卖出\n")
    f.write(f"  5. 不板天数: 连续{best['sd']}天不涨停 → 强制卖出\n")
    f.write(f"\n交易员选股+此规则: {best['final']:,.0f} (距1M: {best['diff']:+,.0f})\n")
    f.write(f"模型选股+此规则: {mfinal:,.0f}\n")
