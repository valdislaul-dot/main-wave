...
out_path=r'C:\Users\Davis\Desktop\项目\gogo\logs\trader_backtest_rerun.txt'
with open(out_path,'w',encoding='utf-8') as f:
    f.write("="*65+"\n")
    f.write("交易员A 回测\n")
    f.write("涨停日卖涨停价 | 不涨停日卖(H+O)/2 | 55%仓位\n")
    f.write("="*65+"\n\n")
    f.write(f"{'阈值':<6} {'最终资产':>12} {'收益率':>10} {'距1022k':>10} {'交易':>5} {'胜率':>6}\n")
    f.write("-"*55+"\n")
    ...
    for threshold in [6,5,4,3]:
        ...
        marker=' <-- closest' if abs(diff)<100000 else ''
        f.write(f"{threshold}%    {final:>12,.0f} {ret:>+9.1f}% {diff:>+10,.0f} {len(tlist):>5} {wr:>5.0f}%{marker}\n")

    f.write("\n卖出: 涨停日卖涨停价(可执行限价单) / 不涨停日卖(H+O)/2\n")
    f.write("买入: 开盘价, 55%仓位\n")
