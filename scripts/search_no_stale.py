...
out_path=r'C:\Users\Davis\Desktop\项目\gogo\search_no_stale_results.txt'
with open(out_path,'w',encoding='utf-8') as f:
    f.write("="*80+"\n")
    f.write("去掉「不板天数强制卖出」后的网格搜索\n")
    f.write("仅用: 连板持有 + 硬止损 + 移动止盈 + 低开退出\n")
    f.write("="*80+"\n\n")

    results=[]

    for hard_stop in [-5,-7,-8,-10,-12,-15]:
        for trail_stop in [-3,-5,-8,-10,-12]:
            for gap_exit in [0,-1,-2,-3,-5]:
                for sp_fn,sp_lbl in [(sp_v,'VWAP'),(sp_c,'CLOSE'),(sp_o,'OPEN')]:
                    ...
                    for didx,date in enumerate(ad):
                        ...
                        for pos in pos_list[:]:
                            ...
                            # Gap exit (only if prev day not LU)
                    ...
                    results.append({'hs':hard_stop,'ts':trail_stop,'ge':gap_exit,'sp':sp_lbl,'final':final,'diff':diff,'ntr':ntr,'wr':wr_})

    results.sort(key=lambda x: abs(x['diff']))

    f.write(f"{'硬止损':>6} {'移动止盈':>8} {'低开退出':>8} {'卖出价':<8} {'最终资产':>12} {'距目标':>10} {'交易':>5} {'胜率':>6}\n")
    f.write("-"*70+"\n")
    for r in results[:25]:
        f.write(f"{r['hs']:>+5}% {r['ts']:>+7}% {r['ge']:>+7}% {r['sp']:<8} {r['final']:>12,.0f} {r['diff']:>+10,.0f} {r['ntr']:>5} {r['wr']:>5.0f}%\n")

    best=results[0]
    f.write(f"\n\n最佳匹配:\n")
    f.write(f"  硬止损={best['hs']}% | 移动止盈={best['ts']}% | 低开退出={best['ge']}% | 卖出价={best['sp']}\n")
    f.write(f"  最终={best['final']:,.0f} | 距1M={best['diff']:+,.0f} ({(best['final']-INIT)/INIT*100:+.1f}%)\n")
    f.write(f"  交易={best['ntr']}笔 | 胜率={best['wr']:.0f}%\n")

    # Now apply to MODEL picks
    f.write(f"\n\n应用最佳规则到模型选股:\n")
    f.write("-"*50+"\n")
    ...
    for didx,date in enumerate(ad):
        ...
        for pos in pos_list[:]:
            ...
            it=pdb[n][date]; ip=pdb[n].get(pd_) if pd_ else None
            if ip and ip['is_limit_up']: pos['pkc']=max(pos.get('pkc',0),it['close']); continue
            pos['pkc']=max(pos.get('pkc',it['close']),it['close'])
            should_sell=False; sp=None
            loss=(it['open']-bp_)/bp_*100
            ...
            if not should_sell and ip:
                ...
                if pcv>0:
                    gap=(it['open']-pcv)/pcv*100
                    if gap<best['ge']: should_sell=True; sp=sp_fn_best(date,n)
            if should_sell and sp and sp>0:
    ...
    # Final summary
    f.write(f"\n\n{'='*80}\n")
    f.write("推断的交易员真实卖出规则 (仅3条)\n")
    f.write(f"{'='*80}\n")
    f.write(f"  1. 连板持有: 前日涨停 → 坚决不卖\n")
    f.write(f"  2. 硬止损: 开盘跌幅超过{abs(best['hs'])}% → 开盘价立即卖出\n")
    f.write(f"  3. 移动止盈: 从最高收盘价回撤超过{abs(best['ts'])}% → {best['sp']}价卖出\n")
    f.write(f"  4. 低开退出: 不涨停+今日低开超过{abs(best['ge'])}% → {best['sp']}价卖出\n")
