...
# ── 模拟交易 (决策树简化版, 与 sell_engine 现行口径同向) ──
def simulate(buy, t_limit_up, t_close, t1):
    """T日开盘买入 → T+1按卖点决策树简化结算 → (卖价, 说明)

    口径说明 (2026-09-17): 本函数是 sell_engine 决策树的**日线简化近似** ——
    低开→竞价卖、大高开≥5%→分歧卖点执行价、其余按 gap 分档持有/开盘卖。
    决策树完整的次日确认(开盘5分钟内破7%、量能对比爆量日、跌破0%/分时均线)需盘中
    分钟级数据, 日线口径无法复现, 故此处只做方向性近似。
    ⚠ 曾于 2026-09-17 一度改为 A式口径(挂涨停价吃封板), 后因 A式停用
      (用户指令"严格按A的卖出资料来")已回退 —— 否则本 KPI 会与生产口径相反。
    """
    gap1 = round((t1['open'] - t_close) / t_close * 100, 2)
    # 硬止损 -10% 无条件全卖(与 sell_engine 同序: 优先于一切分支)
    if t1['low'] <= buy * 0.9:
        return round(buy * 0.9, 3), '硬止损-10%'
    if t_limit_up:
        if gap1 < 0:
            return round(t1['open'], 3), f'昨涨停低开{gap1:+.2f}%→竞价全卖'
        # 2026-09-03修复: 大高开≥5%按分歧卖点(执行价公式)结算, 原一律持有收盘系统性偏高
        if gap1 >= 5:
